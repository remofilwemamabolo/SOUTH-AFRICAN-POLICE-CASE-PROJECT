--South Africa Criminal Database - 30 SQL Questions (Questions and Answers)

--1. Display all records from the CriminalCases table.

SELECT * FROM [SA_Criminal_DB].[dbo].[SA_criminal_database];

--2. Count the total number of cases in the table.

SELECT COUNT(RECORDID) FROM [SA_Criminal_DB].[dbo].[SA_criminal_database]

--3. Count the number of cases in each province.

SELECT COUNT(RECORDID) AS CASES, PROVINCE FROM [SA_Criminal_DB].[dbo].[SA_criminal_database]
GROUP BY PROVINCE
